...
/*
MLFF_call function acts as the interface in the electronicGroundState file. This function is called if (mlff_flag>0)

[Input]
1. pSPARC: SPARC object
[Output]
1. pSPARC: SPARC object with energies, forces, and stresses predicted by MLFF
*/

void MLFF_call(SPARC_OBJ* pSPARC){
	// TODO: Account for socket restarts where elecgs_Count may be reset
	int calc_count = pSPARC->elecgs_Count;
	#ifdef USE_SOCKET
		if (pSPARC->SocketFlag == 1) calc_count = pSPARC->SocketSCFCount - 1;
	#endif
	if (calc_count < 1) {
		init_MLFF_structure(pSPARC);
		init_MLFF_simulation(pSPARC);
	} else {
		// pSPARC->mlff_flag == 1 means on-the-fly MD from scratch, pSPARC->mlff_flag == 21 means only prediction using a known model, pSPARC->mlff_flag == 22 on-the-fly starting from a known model
		if (pSPARC->mlff_flag == 1 || pSPARC->mlff_flag == 22) {
			MLFF_train_predict(pSPARC, pSPARC->mlff_str);
		} else if (pSPARC->mlff_flag == 21) {
			MLFF_only_predict(pSPARC, pSPARC->mlff_str);
		}
...
/*
init_MLFF_structure function initializes and dynamically allocates memory to various objects in MLFF_Obj

[Input]
1. pSPARC: SPARC_OBJ structure
[Output]
1. pSPARC: SPARC_OBJ with MLFF_Obj mlff_str structure initialized
*/
...
t1 = MPI_Wtime();
	...
	// begin formatting the model folder path
    ...
    free(merged_string);
    // end formatting the model folder path
	...
	if (pSPARC->descriptor_typ_MLFF < 2) {
		...
		printf("GMP Descriptor not implemented yet\n");
		exit(1);
	}
...
#pragma GCC diagnostic pop
	...
	free(INPUT_DIR);
	if (pSPARC->descriptor_typ_MLFF>1 && pSPARC->elec_T > 2000){
		if (rank==0){
			printf("WARNING: GMP for high temepratures calculations has not been tested yet! Please use SOAP.\n");
			exit(1);
		}
	}
t2 = MPI_Wtime();
	if (pSPARC->print_mlff_flag == 1 && rank ==0){
		fprintf(mlff_str->fp_mlff, "MLFF object intialized. Time taken: %.3f s\n", t2-t1);
	}
}

/**
 * @brief      Initializes the simulations that will be using the MLFF structure. 
 */
...
void init_MLFF_simulation(SPARC_OBJ *pSPARC) {
	...
	switch (pSPARC->mlff_flag) {
		case 1:
			init_new_MLFF(pSPARC, mlff_str);
			break;
		case 22:
			init_existing_trainable_MLFF(pSPARC, mlff_str);
			break;
		case 21:
			init_static_MLFF(pSPARC, mlff_str);
			break;
		default:
			if (rank == 0) {
				printf("Error: MLFF flag not recognized\n");
				exit(1);
			}
	}
}
...
t2 = MPI_Wtime();
	if (pSPARC->print_mlff_flag == 1 && rank ==0){
		fprintf(mlff_str->fp_mlff, "DFT call done for the first step. Time taken: %.3f s\n", t2-t1);
	}
	...
	if(pSPARC->MDFlag == 1 || pSPARC->RelaxFlag == 1){
		if(pSPARC->cell_typ != 0){
			for(int i = 0; i < pSPARC->n_atom; i++)
				Cart2nonCart_coord(pSPARC, &pSPARC->atom_pos[3*i], &pSPARC->atom_pos[3*i+1], &pSPARC->atom_pos[3*i+2]);
		}
	}
t1 = MPI_Wtime();		
	sparc_mlff_interface_initialDFT(pSPARC, mlff_str);
t2 = MPI_Wtime();
	if (pSPARC->print_mlff_flag == 1 && rank ==0){
		fprintf(mlff_str->fp_mlff, "Covariance matrices formed from first DFT data. Time taken: %.3f s\n", t2-t1);
	}
	if(pSPARC->MDFlag == 1 || pSPARC->RelaxFlag == 1){
		if(pSPARC->cell_typ != 0){
			for(int i = 0; i < pSPARC->n_atom; i++)
...
t2 = MPI_Wtime();
	if (pSPARC->print_mlff_flag == 1 && rank ==0){
		fprintf(mlff_str->fp_mlff, "Existing MLFF ref-atom file read. Time taken: %.3f s\n", t2-t1);
	}
t1 = MPI_Wtime();
...
t2 = MPI_Wtime();
	if (pSPARC->print_mlff_flag == 1 && rank ==0){
		fprintf(mlff_str->fp_mlff, "MLFF ref-atom read data broadcasted. Time taken: %.3f s\n", t2-t1);
	}
	char *fname_str;
	...
	if (rank==0){
		...
		if(pSPARC->cell_typ != 0){
			for (int istr = 0; istr < mlff_str->n_str; istr++){
				// TODO: handling training on a mix of orthogonal and non-orthogonal structures 
				for (int i = 0; i < natom_data[istr]; i++) {
					Cart2nonCart_coord(pSPARC, &apos_data[istr][3*i], &apos_data[istr][3*i+1], &apos_data[istr][3*i+2]);
				}
			}
		}
	}
t2 = MPI_Wtime();
	if (pSPARC->print_mlff_flag == 1 && rank ==0){
		fprintf(mlff_str->fp_mlff, "Read structures data from the exisiting MLFF model. Time taken: %.3f s\n", t2-t1);
	}
t1 = MPI_Wtime();
...
t2 = MPI_Wtime();
	if (pSPARC->print_mlff_flag == 1 && rank ==0){
		fprintf(mlff_str->fp_mlff, "Pretraining from the exisiting data done. Time taken: %.3f s\n", t2-t1);
	}
t1 = MPI_Wtime();
	Calculate_electronicGroundState(pSPARC);
t2 = MPI_Wtime();
	if (pSPARC->print_mlff_flag == 1 && rank ==0){
		fprintf(mlff_str->fp_mlff, "DFT call made after pretraining. Time taken: %.3f s\n", t2-t1);
	}
	MPI_Bcast(pSPARC->forces, 3*pSPARC->n_atom, MPI_DOUBLE, 0, MPI_COMM_WORLD);
	MPI_Bcast(pSPARC->stress, 6, MPI_DOUBLE, 0, MPI_COMM_WORLD);
	MPI_Bcast(&pSPARC->pres, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
...
t1 = MPI_Wtime();
	sparc_mlff_interface_addDFT(pSPARC, mlff_str);
t2 = MPI_Wtime();
	if (pSPARC->print_mlff_flag == 1 && rank ==0){
		fprintf(mlff_str->fp_mlff, "Covariance matrices formed after the DFT call. Time taken: %.3f s\n", t2-t1);
	}
t1 = MPI_Wtime();
	mlff_train_Bayesian(mlff_str);
t2 = MPI_Wtime();
	if (pSPARC->print_mlff_flag == 1 && rank ==0){
		fprintf(mlff_str->fp_mlff, "MLFF model trained with Bayesian regression. Time taken: %.3f s\n", t2-t1);
	}
	pSPARC->last_train_iter = 0;
...
t2 = MPI_Wtime();
	...
	MLFF_only_predict(pSPARC, mlff_str);
t2 = MPI_Wtime();
	if (pSPARC->print_mlff_flag == 1 && rank ==0){
		fprintf(mlff_str->fp_mlff, "MLFF model prediction for E, F, stress done. Time taken: %.3f s\n", t2-t1);
	}
	if(pSPARC->MDFlag == 1 || pSPARC->RelaxFlag == 1){
		if(pSPARC->cell_typ != 0){
			for(int i = 0; i < pSPARC->n_atom; i++)
...
void MLFF_train_predict(SPARC_OBJ *pSPARC, MLFF_Obj *mlff_str){
	...
	// TODO: Allow restart from untrained MLFF model with existing structures and atoms files
	if (pSPARC->mlff_flag==22){
		initial_train = -1;
		if (mlff_str->n_str < 1) {
			printf("Error: Attempting to restart a mlff simulation from pretrained model, but no reference structures found. Exiting...\n");
			exit(1);
		}
	}
	if (pSPARC->print_mlff_flag==1 && rank==0){
		fprintf(fp_mlff,"--------------------------------------------------------------------------------------------\n");
		int print_count = pSPARC->elecgs_Count;
		#ifdef USE_SOCKET
			if (pSPARC->SocketFlag == 1) print_count = pSPARC->SocketSCFCount - 1;
		#endif
		fprintf(fp_mlff, "Calculation: %d\n", print_count);
	}
	int BC[] = {pSPARC->BCx, pSPARC->BCy, pSPARC->BCz};
	int index[] = {0,1,2,3,4,5};
	reshape_stress(pSPARC->cell_typ, BC, index);
	if (mlff_str->n_str <= initial_train && pSPARC->last_train_iter < 0){
t1 = MPI_Wtime();		
		Calculate_electronicGroundState(pSPARC);
		if (pSPARC->MDFlag == 1 || pSPARC->RelaxFlag == 1) {
			if(pSPARC->cell_typ != 0){
...
t2 = MPI_Wtime();
		...
		if (mlff_str->n_str == initial_train){
t1 = MPI_Wtime();
			mlff_train_Bayesian(mlff_str); 
...
t2 = MPI_Wtime();
			...
			pSPARC->last_train_iter = pSPARC->elecgs_Count;
			#ifdef USE_SOCKET
				if (pSPARC->SocketFlag == 1) pSPARC->last_train_iter = pSPARC->SocketSCFCount - 1;
			#endif
		}
		delete_dyarray(&mlff_str->atom_idx_addtrain);	
	} else {
...
t2 = MPI_Wtime();
		if (pSPARC->print_mlff_flag == 1 && rank ==0){
			fprintf(fp_mlff, "MLFF prediction made for the current step. Time taken: %.3f s\n", t2-t1);
		}
		double max_pred_error = fabs(largest(&bayesian_error[1+mlff_str->stress_len], 3*pSPARC->n_atom));
		int train_iter_compare = pSPARC->elecgs_Count;
		#ifdef USE_SOCKET
			if (pSPARC->SocketFlag == 1) train_iter_compare = pSPARC->SocketSCFCount - 1;
		#endif
		if (train_iter_compare == pSPARC->last_train_iter + 1) {
		...
		int Count_Sim = pSPARC->elecgs_Count + pSPARC->restartCount + (pSPARC->RestartFlag == 0);
		#ifdef USE_SOCKET
			if (pSPARC->SocketFlag == 1) Count_Sim = pSPARC->SocketSCFCount - 1 + pSPARC->restartCount + (pSPARC->RestartFlag == 0);
		#endif
		if (max_pred_error >= pSPARC->F_tol_SOAP || !(Count_Sim % pSPARC->MLFF_DFT_fq)){	
			if (pSPARC->print_mlff_flag == 1 && rank ==0){
				if (max_pred_error >= pSPARC->F_tol_SOAP){
					fprintf(fp_mlff, "Error prediction in ML Model is too large!\n");
				} else {
					fprintf(fp_mlff, "Max consecutive MLFF steps reached. Calling DFT.\n");
				}
			}
t1 = MPI_Wtime();
			Calculate_electronicGroundState(pSPARC);
...
/**
 * @brief      This function is used to add the first DFT step to the MLFF training set
 */
...
t1 = MPI_Wtime();
	...
	add_firstDFT(desc_str, nlist, mlff_str, pSPARC->Etot/pSPARC->n_atom, force, stress);
	if (rank==0){
		if(pSPARC->cell_typ != 0){
	        for(int i = 0; i < pSPARC->n_atom; i++)
	            nonCart2Cart_coord(pSPARC, &pSPARC->atom_pos[3*i], &pSPARC->atom_pos[3*i+1], &pSPARC->atom_pos[3*i+2]);	
...
void write_MLFF_results(SPARC_OBJ *pSPARC){
    ...
    if (!rank && pSPARC->Verbosity) {
		...
		if (pSPARC->MDFlag == 1)
			...
			fprintf(output_fp,"                Energy and force calculation (MLFF #%d)                 \n", pSPARC->SocketSCFCount - 1 + pSPARC->restartCount + (pSPARC->RestartFlag == 0));
#endif
		else 
			fprintf(output_fp,"                Energy and force calculation (MLFF #%d)                 \n", 1);
        fprintf(output_fp,"====================================================================\n");
		...
		if (pSPARC->PrintForceFlag == 1 && pSPARC->MDFlag == 0 && pSPARC->RelaxFlag == 0) {
			static_fp = fopen(pSPARC->StaticFilename,"a");
			if (static_fp == NULL) {
				printf("\nCannot open file \"%s\"\n",pSPARC->StaticFilename);
				exit(EXIT_FAILURE);
